...
// Componentes de UI
import { initializeTheme, getThemeFromLocalStorage } from './ui/theme.js';
...
import { populateCustomizationTab } from './views/customizationView.js';
...
function calculateAnalytics(appointmentsData, financialData) {
    ...
    const agendaDoDia = todayAppointments.map(row => ({
        hora: new Date(row.appointment_datetime).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' }),
    ...
    financialData.forEach(entry => {
        ...
        if (entryDate.getFullYear() === currentYear && entryDate.getMonth() === currentMonth) {
            if (entry.value > 0) monthlySummary.revenue += entry.value;
            else monthlySummary.expenses += Math.abs(entry.value);
        }
    });
    ...
    financialData.forEach(entry => {
        ...
        if (entryDate.getFullYear() === currentYear) {
            if (entry.value > 0) annualSummary.revenueData[entryDate.getMonth()] += entry.value;
            else annualSummary.expenseData[entryDate.getMonth()] += Math.abs(entry.value);
        }
    });
    ...
    return {
        ...
        financials: { monthlySummary, annualSummary }
    };
}
    ...
    try {
        ...
        const futureAppointments = appointments
            ...
            .map(app => ({
                ...
                telefone: app.phone,
                data: app.appointment_datetime.split('T')[0],
                hora: new Date(app.appointment_datetime).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' }),
                servico: app.service,
                preco: app.price,
        ...
        const mappedFinancials = {
            monthlySummary: analytics.financials.monthlySummary,
            annualSummary: analytics.financials.annualSummary,
            allExpenses: financialEntries.filter(e => e.value < 0).map(exp => ({
                id: exp.id,
                date: exp.entry_date,
        ...
        // Popula todas as abas com os dados atualizados
        ...
        populateCustomizationTab(handleSaveTheme, handleResetTheme, refreshUI);

        switchTab(activeTabId); // Garante que a aba ativa seja exibida
...
// Handlers que conectam as Views com a API e o Estado
...
async function handleSaveAppointment(event) {
    ...
    const appointmentDate = document.getElementById('edit-appointment-data-wrapper').dataset.selectedValue;
    const appointmentTime = document.getElementById('edit-appointment-hora-wrapper').dataset.selectedValue;
    ...
    const updatedData = {
        ...
        price: document.getElementById('edit-appointment-preco').value,
        professional_name: document.getElementById('edit-appointment-professional-wrapper').dataset.selectedValue,
    };
...
function handleSaveTheme(button) {
    const theme = getThemeFromLocalStorage();
    handleSaveSettings({ theme }, button);
}
