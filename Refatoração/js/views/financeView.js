...
export function populateFinanceTab(financials, onAddExpense, onDelete) {
    ...
    // Gera as linhas da tabela de despesas.
    let expensesHtml = '';
    if (!financials.allExpenses || financials.allExpenses.length === 0) {
        expensesHtml = '<tr><td colspan="5" class="text-center py-4 text-muted">Nenhuma despesa registrada.</td></tr>';
    } else {
        financials.allExpenses.forEach(exp => {
        ...
        <div class="space-y-8">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-5 gap-8">
                ...
                <div class="lg:col-span-3 bg-card p-6 rounded-lg border border-main">
                    <h2 class="text-xl font-semibold mb-4">Detalhes das Despesas</h2>
                    <div class="overflow-x-auto max-h-96 scrollable-list">
                        <table class="w-full text-left">
                            ...
                            <tbody id="expenses-table-body">${expensesHtml}</tbody>
                        </table>
                    </div>
                </div>
            </div>
    ...
    container.querySelector('#add-expense-form').addEventListener('submit', (e) => {
        ...
        onAddExpense(expenseData, e.currentTarget); // Passa os dados e o formulário para o callback.
    });
