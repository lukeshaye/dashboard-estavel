...
+    const spinnerIcon = `<i data-lucide="loader-2" class="w-4 h-4 mr-2 animate-spin"></i>`;
     button.innerHTML = `${spinnerIcon} <span>${text}</span>`;
+    
+    // Re-initialize Lucide icons for the new spinner
...
+        button.innerHTML = `<i data-lucide="check" class="w-4 h-4 mr-2"></i> <span>${feedbackText}</span>`;
     } else {
...
+        button.innerHTML = `<i data-lucide="x" class="w-4 h-4 mr-2"></i> <span class="truncate">${feedbackText}</span>`;
     }
...
+            
+            // Re-initialize Lucide icons for restored content
+            if (typeof lucide !== 'undefined') {
+                lucide.createIcons();
+            }
         }
